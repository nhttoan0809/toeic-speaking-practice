...
export default function ContentArea({ section, activeTab, onTabChange, onSectionChange }: ContentAreaProps) {
  return (
            ...
            >
             <div className="w-24 h-24 bg-primary/10 rounded-3xl flex items-center justify-center text-primary mb-8">
                {section.icon ? (
                  ...
                  section.module === 'speaking' ? <Mic2 className="w-10 h-10" /> : <PenTool className="w-10 h-10" />
                )}
             </div>
             ...
             <p className="text-slate-500 max-w-xl mx-auto text-lg leading-relaxed">
               ...
               Tại đây, chúng tôi cung cấp đầy đủ các kiến thức nền tảng, mẹo làm bài và hệ thống bài tập chuyển sâu cho từng phần thi.
             </p>
             ...
             <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-4xl">
              {[
                ...
                <div key={idx} className="p-6 bg-slate-50 border border-slate-100 rounded-2xl flex flex-col items-center">
                  <span className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-1">{stat.label}</span>
             ...
             <button 
              onClick={() => {
                  const firstContentId = SECTIONS.find(s => s.module === section.module && s.type === 'content')?.id;
