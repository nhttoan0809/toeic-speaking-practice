...
interface TestData {
  id: string;
  title: string;
  part: SpeakingPart5Item | null;
}

interface ExamResponse {
  id: string;
  title: string;
  speaking_part_5: SpeakingPart5Item[];
}

export default function SpeakingPart5Resources() {
      ...
      try {
        ...
        setTests(
          sortedData.map((item) => ({
            id: item.id,
            title: item.title,
            part: item.speaking_part_5[0] ?? null,
          })),
        );
      } catch (err: unknown) {
        console.error('Error fetching Speaking Part 5 data:', err);
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-slate-400">
        ...
        <p className="font-medium">Đang tải tài liệu Part 5...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex items-center justify-between mb-2">
        <div>
          <h3 className="text-2xl font-black text-slate-900">Nêu ý kiến</h3>
          <p className="text-slate-500 mt-1">Tổng cộng {tests.length} bộ đề thi Speaking Part 5</p>
        </div>
          ...
          >
            ...
            {test.part ? (
              <div className="bg-slate-50 border border-slate-100 rounded-2xl p-6 md:p-8">
                <span className="text-[10px] font-black text-primary/60 uppercase tracking-widest block mb-4">
                  Đề bài (Prompt)
                </span>
                <p className="text-slate-700 leading-relaxed text-base md:text-lg font-bold">
                  {test.part.prompt}
                </p>
              </div>
            ) : (
