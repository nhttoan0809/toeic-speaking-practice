...
interface TestData {
  id: string;
  title: string;
  part: SpeakingPart2Item | null;
}

interface ExamResponse {
  id: string;
  title: string;
  speaking_part_2: SpeakingPart2Item[];
}

export default function SpeakingPart2Resources() {
      ...
      try {
        ...
        setTests(
          sortedData.map((item) => ({
            id: item.id,
            title: item.title,
            part: item.speaking_part_2[0] ?? null,
          })),
        );
      } catch (err: unknown) {
        console.error('Error fetching Speaking Part 2 data:', err);
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-slate-400">
        ...
        <p className="font-medium">Đang tải tài liệu Part 2...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex items-center justify-between mb-2">
        <div>
          <h3 className="text-2xl font-black text-slate-900">Mô tả tranh</h3>
          <p className="text-slate-500 mt-1">Tổng cộng {tests.length} bộ đề thi Speaking Part 2</p>
        </div>
          ...
          >
            ...
            {test.part ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {test.part.image_1_url && (
                  <div className="space-y-3">
                    <span className="text-xs font-black text-primary/60 uppercase tracking-widest block">
                      Bức tranh 1
                    </span>
                    <div className="relative aspect-4/3 rounded-2xl overflow-hidden bg-slate-100 border border-slate-200">
                      <img
                        src={test.part.image_1_url}
                        alt={`${test.title} - Image 1`}
                ...
                {test.part.image_2_url && (
                  <div className="space-y-3">
                    ...
                    <div className="relative aspect-4/3 rounded-2xl overflow-hidden bg-slate-100 border border-slate-200">
                      <img
                        src={test.part.image_2_url}
                        alt={`${test.title} - Image 2`}
                        className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
                        loading="lazy"
                      />
                    </div>
