    ...
    { name: 'Speaking & Writing', path: '/speaking-writing', type: 'internal' },
    {
      ...
      items: [
        ...
        },
        {
          ...
          id: 'vocab',
          items: [
            { name: 'Workplace', url: 'https://mssmiletoeic.my.canva.site/vocabulary-boost-for-toeic-workplace-theme-sw', type: 'external' },
            { name: 'Personal Tastes', url: 'https://mssmiletoeic.my.canva.site/vocabulary-boost-for-toeic-personal-tastes-sw', type: 'external' },
            { name: 'Everyday Life', url: 'https://mssmiletoeic.my.canva.site/vocabulary-boost-for-toeic-sw-everyday-life-theme', type: 'external' },
          ]
        }
      ]
    }
    ...
    <header className="bg-white shadow-sm border-b border-slate-100 sticky top-0 z-50">
      <div className="container mx-auto px-4 py-4 flex justify-between items-center">
        ...
        <nav className="hidden md:flex items-center space-x-8">
          {navLinks.map((link) => (
            <div key={link.name} className="relative py-2" // Added padding Y to increase hover area
                 onMouseEnter={() => link.type === 'dropdown' && setActiveDropdown(link.id || null)}
              ...
              {link.type === 'internal' ? (
                ...
                <button 
                  className={`flex items-center gap-1 font-bold transition-colors text-sm uppercase tracking-wide focus:outline-none ${
                    activeDropdown === link.id ? 'text-primary' : 'text-slate-600 hover:text-primary'
                  }`}
                >
                  {link.name}
                  <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${activeDropdown === link.id ? 'rotate-180' : ''}`} />
                </button>
              )}
                    ...
                    {link.items?.map((item) => (
                      <div key={item.name} className="relative group/sub w-full"
                          ...
                          // but usually nested heavy UI relies on CSS hover or careful JS.
                           ...
                           onMouseLeave={() => item.type === 'sub-dropdown' && setActiveSubDropdown(null)}
                      >
                        {item.type === 'external' ? (
                          <a
                            href={item.url}
          ...
          <a href="#" className="hidden lg:block bg-accent hover:bg-accent-hover text-white px-5 py-2.5 rounded-full font-bold text-sm shadow-md shadow-orange-200 transition-all hover:scale-105 active:scale-95">
            Đăng ký học thử
          </a>
        </nav>
        ...
        {isMobileMenuOpen && (
            ...
            >
                ...
                {navLinks.map((link) => (
                    <div key={link.name}>
                    {link.type === 'internal' ? (
                        <Link to={link.path!} className="block text-slate-800 font-bold text-lg" onClick={() => setIsMobileMenuOpen(false)}>
                        {link.name}
                        </Link>
                    ) : (
                        ...
                        {link.items?.map(item => (
                            ...
                            {item.type === 'external' ? (
                                <a href={item.url} target="_blank" className="block text-slate-600 font-medium py-1">{item.name}</a>
                            ) : (
                                <div className="space-y-1">
                                <div className="text-slate-800 font-semibold py-1">{item.name}</div>
                                {item.items?.map(sub => (
                                    <a key={sub.name} href={sub.url} target="_blank" className="block pl-4 text-slate-500 text-sm py-1">{sub.name}</a>
                                ))}
                                </div>
                            )}
                            </div>
